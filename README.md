...
Describe schemas in human-readable terms. Useful for understanding unfamiliar schemas or for AI coding agents that need schema context.
...
Output includes field names, types, documentation, nullability, defaults, and referenced schemas with their fields.

### Schema Suggestions

Propose compatible schema changes from a natural language description. Knows Avro compatibility rules and type promotion.
...
For breaking changes, shows why it breaks and suggests safe alternatives.

### Schema Generation

Infer schemas from sample JSON data. Supports Avro, Protobuf, and JSON Schema output.
...
Detects common string formats (ISO dates, UUIDs, emails) and annotates them in the generated schema.
